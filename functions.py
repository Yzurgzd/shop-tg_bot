...
class DataBase:
	def __init__(self, path="users.sqlite3"):
	...
	def new_user(self, chat_id, username, referer=None):
		"""
		Запись нового юзера
		"""
		...
		values = (id, chat_id, username, 0, round(time.time()), referer)
	...
	def get_all_users(self):
		...
		self.cursor.execute(sql)
		for id, chat_id, username, sum_pay, reg_time, referer in self.cursor.fetchall():
			reg_time = datetime.datetime.fromtimestamp(reg_time)

			mes = "ID в боте: {}\nChat_id: {}\nUsername: @{}\nСумма пополнений: {}\nДата регистрации: {}\nРеферер: {}"
			mes = mes.format(id, chat_id, username, sum_pay, reg_time, referer)
